...
enum APIError: LocalizedError {
    ...
    var diagnostic: String {
        ...
        case .unauthorized(let detail):
            return detail.isEmpty ? "401" : "401 · \(detail)"
...
actor APIClient {
    ...
    private func raw(_ method: String, _ path: String, body: Data?) async throws -> Data {
        ...
        if http.statusCode == 401 {
            ...
            let refreshed = await OIDCService.shared.refresh(tenant: tenant)
            if refreshed {
                ...
                if let retryHTTP = retryResponse as? HTTPURLResponse {
                    ...
                    throw APIError.unauthorized(Self.detail(from: retryData))
                }
            }
            throw APIError.unauthorized(
                refreshed ? detail : "sign in again (token refresh failed) · \(detail)"
            )
        }
