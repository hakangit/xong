...
final class AppConfiguration: ObservableObject {
    ...
    func refreshTenant() async {
        guard case .server(let current) = mode,
              var components = URLComponents(url: current.apiBase, resolvingAgainstBaseURL: false)
        else { return }

        components.path = TenantDiscovery.wellKnownPath
        components.query = nil
        guard let url = components.url,
              let fresh = try? await TenantDiscovery.fetch(url),
              fresh != current else { return }

        connect(to: fresh, as: identity)
    }
